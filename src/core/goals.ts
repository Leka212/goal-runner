...
import { evaluateDoneGate } from "./gates.js";
...
export async function stopGoal(root: string, slug: string, status: GoalStatus): Promise<void> {
  ...
  if (status === "done") {
    const gate = await evaluateDoneGate(root, slug);
    if (!gate.ok) throw new Error(gate.reasons.join("; "));
    data.gate_provenance = gate.provenance;
  }
  await recordEvent(root, { type: "goal.stopped", slug, data });
