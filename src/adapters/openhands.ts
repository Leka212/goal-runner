export function renderOpenHandsGuide(goalTitle: string): string {
  return `# OpenHands Goal Protocol guide

Goal: ${goalTitle}

This is generate-only local guidance for OpenHands repository instructions or microagent context. Use it as workspace instruction text only; the Goal Protocol CLI remains the source of truth for goal state, gates, redaction, verification, and evidence reporting hooks.
...
- Keep OpenHands instructions and artifacts in the current workspace unless Goal Protocol configuration and review gates explicitly allow more.
