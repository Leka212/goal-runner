export function renderContinueDevGuide(goalTitle: string): string {
  return `# Continue.dev Goal Protocol guide

Goal: ${goalTitle}

This is generate-only local guidance for Continue.dev rules or chat context. Use it as workspace instruction text only; the Goal Protocol CLI remains the source of truth for goal state, gates, redaction, verification, and evidence reporting hooks.
...
- Keep Continue.dev rules and artifacts in the current workspace unless Goal Protocol configuration and review gates explicitly allow more.
